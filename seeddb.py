...
with open("static/covidcases.csv", newline="") as csvfile:
    ...
    for row in csv_reader:
        ...
        print(
            ...
            row["tot_deaths"],
            row["new_deaths"],
            row["new_historic_cases"],
            row["new_historic_deaths"],
        )
        ...
        row["end_date"] = (
            row["end_date"][6:] + "-" + row["end_date"][:2] + "-" + row["end_date"][3:5]
        )

        cursor.execute(
            "INSERT INTO covid_data (date_updated, state, start_date, end_date, tot_cases, new_cases, tot_deaths, new_deaths, new_historic_cases, new_historic_deaths) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            (
                row["date_updated"],
                row["state"],
                row["start_date"],
