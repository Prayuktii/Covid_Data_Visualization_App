...
config = {
    ...
    "port": int(
        os.environ.get("DB_PORT", "3360")
    ),  # Make sure to use an integer for the port
    "database": os.environ.get("DB_NAME"),
}


def retivecases(
    filter_state: str = None, filter_start_date: str = None, filter_end_date: str = None
) -> List[Dict]:
...
def dates():
    ...
    dates_data = [
        {"start_date": row[0].isoformat(), "end_date": row[1].isoformat()}
        for row in results
    ]
